...
const main = async () => {
    ...
    const setupPath = path.resolve(__dirname, 'setup_supabase_storage.sql');
    const sql = await fs.readFile(setupPath, 'utf8');

    try {
        await pool.query(sql);
        console.log('Bucket de imagens do Supabase configurado com sucesso.');
    } finally {
        await pool.end();
    }
};
...
main().catch((error) => {
    console.error('Falha ao configurar o bucket do Supabase:');
