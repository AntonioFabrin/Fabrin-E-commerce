...
export default function CartHeaderIcon({ compact = false }: { compact?: boolean }) {
  const { totalItems } = useCart();
  const router = useRouter();
  const label = totalItems > 0 ? `${totalItems} ite${totalItems > 1 ? 'ns' : 'm'}` : 'Carrinho';

  return (
    <button
      className={compact ? 'cart-icon-button cart-icon-button--compact' : 'cart-icon-button'}
