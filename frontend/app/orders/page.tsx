...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 12px', borderRadius: 'var(--radius-pill)', background: c.bg, color: c.color, border: `1px solid ${c.border}`, fontSize: 11, fontWeight: 700, letterSpacing: '0.3px', whiteSpace: 'nowrap' }}>
...
function ReviewModal({ orderId, productId, productName, onClose, onSuccess }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/api/reviews', { product_id: productId, order_id: orderId, rating, comment });
      onSuccess();
    } catch (err) {
      setError(extractErrorMessage(err, 'Erro ao enviar avaliação.'));
    } finally { setSubmitting(false); }
  };

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(26,10,46,0.7)', backdropFilter: 'blur(8px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '40px 36px', width: '100%', maxWidth: 440, boxShadow: '0 32px 80px rgba(45,20,87,0.2)' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 700, color: 'var(--royal)' }}>Avaliar Produto</h2>
          <button onClick={onClose} style={{ background: 'var(--mist)', border: 'none', cursor: 'pointer', width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: 'var(--muted)' }}>✕</button>
        </div>
        <p style={{ fontSize: 13, color: 'var(--violet)', fontWeight: 600, marginBottom: 28, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{productName}</p>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{ background: 'var(--cream)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '24px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.6px' }}>Sua nota</p>
            <StarPicker value={rating} onChange={setRating} />
          </div>
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.6px', display: 'block', marginBottom: 8 }}>Comentário (opcional)</label>
            <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Como foi o produto? O que você achou?" maxLength={500} rows={4}
              style={{ width: '100%', padding: '12px 16px', background: '#F9F5FF', border: '1.5px solid var(--mist)', borderRadius: 'var(--radius-md)', fontSize: 14, color: 'var(--ink)', outline: 'none', fontFamily: 'var(--font-body)', resize: 'vertical', transition: 'all 0.2s' }}
          ...
          {error && <div style={{ padding: '10px 14px', background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 'var(--radius-md)', fontSize: 13, color: '#DC2626' }}>⚠ {error}</div>}
          <div style={{ display: 'flex', gap: 10 }}>
            <Button type="button" variant="ghost" size="md" style={{ flex: 1 }} onClick={onClose}>Cancelar</Button>
            <button type="submit" disabled={submitting || rating === 0}
              style={{ flex: 1, padding: '10px 20px', background: submitting || rating === 0 ? 'var(--mist)' : '#059669', color: submitting || rating === 0 ? 'var(--muted)' : '#fff', border: 'none', borderRadius: 'var(--radius-pill)', fontSize: 13, fontWeight: 600, cursor: submitting || rating === 0 ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-body)', transition: 'all 0.2s' }}>
    ...
    >
      <div onClick={() => setOpen(!open)} style={{ padding: '20px 24px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', background: open ? 'var(--cream)' : 'transparent', transition: 'background 0.2s' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            {isSeller && order.buyer_name && <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 2 }}>Comprador: <strong style={{ color: 'var(--royal)', fontWeight: 600 }}>{order.buyer_name}</strong></p>}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <p style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 700, color: 'var(--royal)', lineHeight: 1 }}>R$ {Number(order.total).toFixed(2).replace('.', ',')}</p>
          </div>
          <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--mist)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: 'var(--muted)', flexShrink: 0, transform: open ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.2s' }}>▼</div>
        </div>
      </div>
      ...
      {open && (
        <div style={{ borderTop: '1px solid var(--border)' }}>
              ...
              return (
                <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', background: 'var(--cream)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
                  ...
                  {!isSeller && isPaid && onReview && (
                    alreadyReviewed
                      ? <span style={{ fontSize: 11, fontWeight: 700, color: '#D97706', background: '#FFFBEB', border: '1px solid #FDE68A', padding: '5px 12px', borderRadius: 'var(--radius-pill)', whiteSpace: 'nowrap', flexShrink: 0 }}>⭐ Avaliado</span>
                      : <button onClick={() => onReview(order.id, item.product_id, item.product_name)}
                          style={{ fontSize: 12, fontWeight: 600, color: 'var(--muted)', background: 'var(--white)', border: '1.5px solid var(--border)', padding: '6px 14px', borderRadius: 'var(--radius-pill)', cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'var(--font-body)', whiteSpace: 'nowrap', flexShrink: 0 }}
          ...
          <div style={{ margin: '0 24px 20px', padding: '14px 20px', background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
            <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
              {order.payment_method && <span style={{ fontSize: 12, color: 'var(--muted)', display: 'flex', alignItems: 'center', gap: 5 }}>💳 <span style={{ color: 'var(--royal)', fontWeight: 500 }}>{order.payment_method}</span></span>}
              {isSeller && order.buyer_email && <span style={{ fontSize: 12, color: 'var(--muted)', display: 'flex', alignItems: 'center', gap: 5 }}>✉️ <span style={{ color: 'var(--royal)', fontWeight: 500 }}>{order.buyer_email}</span></span>}
              {order.external_reference && <span style={{ fontSize: 11, color: '#C4B5D4', fontFamily: 'monospace' }}>{order.external_reference}</span>}
            </div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
              <span style={{ fontSize: 12, color: 'var(--muted)' }}>Total pago</span>
              <span style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 700, color: 'var(--royal)' }}>R$ {Number(order.total).toFixed(2).replace('.', ',')}</span>
            </div>
          </div>
        </div>
      )}
...
function PaymentReturn({ status }: { status: PageStatus }) {
  ...
  const c = cfgs[status!];
  return (
    <div style={{ background: c.bg, border: `1.5px solid ${c.border}`, borderRadius: 'var(--radius-lg)', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: 16, marginBottom: 28 }}>
...
function OrdersContent({ authLoading }: { authLoading: boolean }) {
  ...
  if (authLoading || dataLoading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 16 }}>
      <Spin /><p style={{ fontSize: 14, color: 'var(--muted)' }}>Carregando pedidos...</p>
    </div>
  );
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '48px 24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 36, flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 700, color: 'var(--royal)' }}>Meus Pedidos</h1>
        </div>
        ...
        {(['comprador', 'vendedor'] as const).map(t => {
          ...
          return (
            <button key={t} onClick={() => setTab(t)}
              style={{ padding: '9px 22px', borderRadius: 9, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer', background: active ? 'var(--violet)' : 'transparent', color: active ? '#F3E8FF' : 'var(--muted)', fontFamily: 'var(--font-body)', display: 'flex', alignItems: 'center', gap: 8, transition: 'all 0.2s' }}>
              {t === 'comprador' ? '🛒 Minhas Compras' : '📦 Vendas'}
              {count > 0 && <span style={{ fontSize: 10, fontWeight: 700, background: active ? 'rgba(255,255,255,0.25)' : 'var(--mist)', color: active ? '#F3E8FF' : 'var(--violet)', padding: '1px 7px', borderRadius: 999 }}>{count}</span>}
            </button>
          );
        })}
      </div>
      ...
      {currentOrders.length > 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {currentOrders.map(order => (
            <OrderCard key={order.id} order={order} isSeller={tab === 'vendedor'} onReview={(oId, pId, pName) => setReviewModal({ orderId: oId, productId: pId, productName: pName })} reviewedSet={reviewedSet} />
          ))}
        </div>
      ) : (
        <div style={{ border: '2px dashed var(--border)', borderRadius: 'var(--radius-lg)', padding: '72px 24px', textAlign: 'center' }}>
          <p style={{ fontSize: 52, marginBottom: 16 }}>{tab === 'comprador' ? '🛍️' : '📦'}</p>
          <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 22, color: 'var(--royal)', marginBottom: 8 }}>{tab === 'comprador' ? 'Nenhuma compra ainda' : 'Nenhuma venda ainda'}</h3>
          <p style={{ fontSize: 14, color: 'var(--muted)', marginBottom: 28, maxWidth: 360, margin: '0 auto 28px' }}>{tab === 'comprador' ? 'Explore nossa loja e faça sua primeira compra!' : 'Quando alguém comprar seus produtos, os pedidos aparecerão aqui.'}</p>
          {tab === 'comprador' && <Link href="/products" style={{ textDecoration: 'none', display: 'inline-block' }}><Button variant="primary" style={{ width: 'auto', padding: '12px 28px' }}>Explorar Loja →</Button></Link>}
        </div>
      )}
    </div>
  );
...
export default function OrdersPage() {
  ...
  return (
    <Suspense fallback={<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 16, paddingTop: 80 }}><Spin /><p style={{ fontSize: 14, color: 'var(--muted)' }}>Carregando...</p></div>}>
      <OrdersContent authLoading={authLoading} />
    </Suspense>
  );
}
